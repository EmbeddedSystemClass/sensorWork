...
class TwoWire
{
  ...
  public:
	...
	inline void send(unsigned long n) { return send((uint8_t)n); }
    inline void send(long n) { return send((uint8_t)n); }
    inline void send(unsigned int n) { return send((uint8_t)n); }
    //inline void send(int n) { return send((uint8_t)n); }
