/*  
 *  ------  GPRS_SIM908_14 - Transparent TCP client in single connection  -------- 
 *  
 *  Explanation: This example shows how to create a transparent TCP client 
...
void loop()
{
    ...
    if ((answer == 1) || (answer == -3))
    {
        ...
        if (answer == 1)
        {
            ...
            answer = GPRS_SIM908.configureGPRS_TCP_UDP(SINGLE_CONNECTION, TRANSPARENT);
            if (answer == 1)
            {
                USB.println(F("Done"));
                ...
                answer = GPRS_SIM908.createSocket(TCP_CLIENT, "IP", "port");
                if (answer == 1)
                {
