/*  
 *  ------ [OTA_05] - OTA basic program with XBee 868 -------- 
 *  
...
{
  USB.ON();
  USB.println("OTA_05_868 example");
  ...
  xbee868.checkNewProgram();  
}

void loop()
{
  // Check if new data is available
  if( xbee868.available() )
  {
    xbee868.treatData();
    // Keep inside this loop while a new program is being received
    while( xbee868.programming_ON  && !xbee868.checkOtapTimeout() )
    {
      if( xbee868.available() )
      {
        xbee868.treatData();
