...
// *****************************************************************************************
  ...
  if (result != 0) {
    ...
    USB.println("Communication error. Couldn't read from temperature sensor");
    delay(100);
  } 
  else { 

    // If all ok 
    USB.print("Temperature read from slave ID 1: ");
...
// *****************************************************************************************
  ...
  if (result != 0) {
    // If no response from the slave, print an error message. 
    USB.println("Communication error. Couldn't read from the C02 sensor");
    delay(100);
  } 
  else { 

    // If all ok. 
    USB.print("CO2 data read from slave ID 2: ");
