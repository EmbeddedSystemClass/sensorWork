/*  
 *  ------ [Ev_3] - Socket 3 Reading for Gases v20-------- 
 *  
 *  Explanation: Turn on the Events Board v20 configuring the threshold
 *  of the socket 3 stage and wait for an interruption from this sensor
...
void setup()
{
  ...
  // Configure the socket 3 threshold
...
void loop()
{
  ...
  value = SensorEventv20.readValue(SENS_SOCKET3);
  ...
  value = SensorEventv20.readValue(SENS_SOCKET3, SENS_RESISTIVE);
...
void interrupt_function()
{
  ...
  // In case the interruption came from socket 3
  if( SensorEventv20.intFlag & SENS_SOCKET3)
  {
    USB.println(F("-----------------------------"));
    USB.println(F("Interruption from socket 3"));
    USB.println(F("-----------------------------"));
  }
