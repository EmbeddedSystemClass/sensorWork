/*  
 *  --[SM_5] - Reading the Flow Sensor connected to the 3.3V socket on Smart Metering v20 board-- 
 *  
 *  Explanation: Turn on the flow sensor every second, taking a measurement and
...
void loop()
{
  ...
  value = SensorSmartv20.readValue(SENS_SMART_FLOW_3V3, SENS_FLOW_YFS401);
  
  // Turn off the sensor
  SensorSmartv20.setSensorMode(SENS_OFF, SENS_SMART_FLOW_3V3);
  
  // Part 2: USB printing
  // Print the flow value through the USB
  USB.print(F("Flow: "));
  USB.print(value);
  USB.println(F("l/min"));
