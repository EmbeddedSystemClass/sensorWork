...
void Adafruit_LEDBackpack::setBrightness(uint8_t b) {
  ...
  Wire.send(HT16K33_CMD_BRIGHTNESS | b);
...
void Adafruit_LEDBackpack::writeDisplay(void) {
  ...
  for (uint8_t i=0; i<8; i++) {
    Wire.send(displaybuffer[i] & 0xFF);    
    Wire.send(displaybuffer[i] >> 8);    
  }
  Wire.endTransmission();  
}
...
void Adafruit_7segment::writeColon(void) {
    Wire.beginTransmission(i2c_addr);
    Wire.send((uint8_t)0x04); // start at address $02
    
    Wire.send(displaybuffer[2] & 0xFF);
    Wire.send(displaybuffer[2] >> 8);
